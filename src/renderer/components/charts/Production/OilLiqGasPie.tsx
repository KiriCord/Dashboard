...
export const OilLiqGasPie = (props: MerProps) => {
    const gas = props.dataMer.map(item => item["gas"]).reduce((a, b) => a + b, 0);
    const oil = props.dataMer.map(item => item["oil"]).reduce((a, b) => a + b, 0);
    const liq = props.dataMer.map(item => item["liq"]).reduce((a, b) => a + b, 0);
    const data = [
        { name: 'Газ', value: gas },
